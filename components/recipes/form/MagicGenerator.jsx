...
export function MagicGenerator({ onDraftGenerated }) {
  ...
  return (
    <div className="relative p-5 border border-emerald-200 dark:border-emerald-800 rounded-xl bg-emerald-50/50 dark:bg-emerald-900/20 shadow-sm transition-all hover:shadow-md">

      {/* Loading Overlay: Prevents interaction during async operation */}
      {isGenerating && (
        <div className="absolute inset-0 bg-white/60 dark:bg-gray-900/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center rounded-xl transition-all">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-600 dark:border-emerald-400 mb-3"></div>
          <span className="text-sm font-semibold text-emerald-800 dark:text-emerald-300 animate-pulse">
      ...
      <p className="text-sm text-emerald-800/80 dark:text-emerald-300/80 mb-4 leading-relaxed">
      ...
      <div className="space-y-3">
        <textarea
          ...
          className="w-full px-4 py-3 border border-emerald-200 dark:border-emerald-800 rounded-lg shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 resize-none"
        />

        {error && (
          <div className="text-xs text-red-600 dark:text-red-400 font-medium pl-1 animate-in fade-in slide-in-from-top-1">
            {error}
          </div>
        )}
        ...
        <div className="flex justify-end">
          <Button
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-200 dark:shadow-none text-xs px-5 border-transparent"
          >
